import { ChampionId, ChampionMasteryDTO, ChampionMasteryDTOSchema, RiotApiClient } from '../../../src';
...
describe('RiotApiClient.leagueOfLegends.championMasteries', () => {
    ...
    test('#championMasteries.scoresBySummoner() is number', async () => {
        const summoner = await getSummoner(client);
        const data: number = await client.lol.championMasteries.scoresBySummoner(summoner);
    ...
    test('#scoresBySummoner() returns valid schema', async () => {
        const summoner: Summoner = await getSummoner(client);
        const mastery = await client.lol.championMasteries.scoresBySummoner(summoner);
